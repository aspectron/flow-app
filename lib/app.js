...
class FlowApp extends EventEmitter{
	constructor(options={}){
		...
		this.binFolder = path.join(options.appFolder,'bin',utils.platform);
		this.init();
	}
	...
	*/
	getDefaultConfig(){
		let file = path.join(this.appFolder, ".defaults");
	...
	initRPCHooks(rpc){
		...
		this._defaultRPCHooks = {
			...
			'get-app-data':(args, callback)=>{
				let {config, configFolder, appFolder, binFolder, dataFolder} = this;
				let {modules} = config;
				callback(null, {config, configFolder, modules, appFolder, binFolder, dataFolder})
			},
			'set-app-theme':(args)=>{
				let {theme} = args;
				if(!theme)
	...
	*/
	saveI18nEntries(entries){
		if(!fs.existsSync(this.getI18nFilePath('i18n.lock')) || /program\sfiles/i.test(__dirname)) {
			fs.writeFileSync(
